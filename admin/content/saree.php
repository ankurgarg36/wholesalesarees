...
if($_REQUEST['prid']==1)
{
/*echo $e=substr(md5('ankur_ins@south.du.ac.in'),0,4);	
echo "<br>".$pass=sha1($e);
*/	 ?>
     <div class="bottom">
     <div class="right_panel">
        ...
        <div class="logo_info" id="image_head">
         <p id="item_image1">Saree Management</p>
       <a onclick="history.back(); return false;" href="#">
           ...
           <span style="float:right;margin-right:140px;"> <a href="dashboard.php?page=saree&prid=2" > Add Saree  </a> </span>              
</div>
         <!-- table starts -->
      <div class="table_box">
    	    <!-- ADD link 
...
<table cellpadding="2" cellspacing="2" border="0" class="display" id="example" width="100%" bgcolor="#2f8be8" >
	...
	$data=mysqli_query($link,"SELECT * FROM tbl_saree") or die(mysql_error());
	$i=1;
	...
	{	
		...
		$opr="dashboard.php?page=saree&prid=4&x=".$id;
	  ?>
      <tr>
...
<img src="resize.php?w=100&h=100&img=../../<?php echo $info['path_to_file']; ?>" title="<?php echo $info['title'];?>" alt='' />	  
      ...
      <td valign="top" >
		 <a href="dashboard.php?page=saree&prid=2&x=<?php echo $id ?>" /> <img src="../images/edit.png" alt="Edit" title="Edit"> </a> |
...
if($_REQUEST['prid']==2)
{
 if(isset($_REQUEST['x']))
	{
		$id=$_REQUEST['x'];
 		$result = mysqli_query($link,"select * from tbl_saree where id='$id'") or die(mysql_error());
		$info = mysqli_fetch_array($result);
		$button="Edit";
		$button_id="submit1";
		$label="Edit Saree Information";
		$action="update";
	}
	else
	{
...
<div class="bottom">
     ...
     <a onclick="history.back(); return false;" href="#">
      ...
      <form name="entry_form" id="entry_form" action="dashboard.php?page=saree&prid=3"  method="post" enctype="multipart/form-data">
      
<table width="70%" border="0" align="center">
  <tr>
    <td height="30" colspan="4" class="strip1">SAREE INFORMATION</td>
  </tr>
<tr>
...
<table width="100%" border="1" align="center" style="border:0px;!important;">
  ...
  <tr>
    ...
    <td width="60"><?php $fnc->getProductMaterial('material',$info['material'],1,'required','235px'); ?></td>
  </tr>  
  <tr>
  ...
  <tr>
    ...
    <td width="60"><?php $fnc->getProductColor('color',$info['color'],1,'required','235px'); ?></td>
  </tr>   
  <tr>
...
<td width="30%;">
<img src="<?php if($info['path_to_file']!='') echo '../../'.$info['path_to_file'];else echo '../../images/show/s4.jpeg';?>" alt="" width="350" height="330" id="img_prev">
...
if($_REQUEST['prid']==3)
{
	...
	if($c_value[1]=="")
	{
				$query=mysqli_query($link,"SELECT max(product_code) FROM tbl_saree")or die(mysql_error());
...
if($file_img!="")
{
			...
			$filepath= "../../uploads/saree/".$new_file_name.basename( $_FILES['image']['name']);
			$path_to_file= "uploads/saree/".$new_file_name.basename( $_FILES['image']['name']);
...
$path_to_file=htmlspecialchars(mysql_escape_string($_POST['hidden_path_to_file']));	
	if($action=="add")
	{
 		$sql="INSERT INTO tbl_saree (product_code,title,material,price,color,purpose,description,meta_keywords,
	...
	{
		$id=$c_value[1];
 		 $sql = "UPDATE  tbl_saree SET title = '$title',material = '$material',price = '$price',color = '$color',
	...
	?>
		...
		alert(msg);
		window.location="dashboard.php?page=saree&prid=1";
		</script>
<?php		
}
/* Delete the record */
...
{
		$id=$_REQUEST['x'];
 		 $sql = "DELETE from tbl_saree WHERE id = '$id'" ;	
		$query= mysqli_query($link,$sql) or die(mysql_error());
		if($query==1)

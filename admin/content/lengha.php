...
<table cellpadding="2" cellspacing="2" border="0" class="display" id="example" width="100%" bgcolor="#2f8be8" >
	...
	$data=mysqli_query($link,"SELECT * FROM tbl_lengha") or die(mysql_error());
	$i=1;
...
if($_REQUEST['prid']==2)
{
 if(isset($_REQUEST['x']))
	{
		$id=$_REQUEST['x'];
 		$result = mysqli_query($link,"select * from tbl_lengha where id='$id'") or die(mysql_error());
...
if($_REQUEST['prid']==3)
{
	...
	if($c_value[1]=="")
	{
				$query=mysqli_query($link,"SELECT max(product_code) FROM tbl_lengha")or die(mysql_error());

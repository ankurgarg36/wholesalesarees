        ...
        <h3>Search Sarees</h3>
         <form method="post" id="form2" class="form1">
           <label >
			...
			<?php
$cat_type=mysqli_query($link,"select * from  tbl_material where product_type='1'") or die(mysql_error());
	...
	echo "<option value='$val[id]' $sel>$val[material_type]</option>";			
          ...
          <label >
            ...
            <select name="color" id="color" onchange="refreshPage(this.value,this.id)">
              <option value="" selected="selected" >All</option>
			<?php
$color=mysqli_query($link,"select * from  tbl_color where product_type='1'") or die(mysql_error());

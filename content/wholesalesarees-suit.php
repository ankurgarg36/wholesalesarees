...
function refreshPage(val,id)
{
	if(val!='')
	{
		...
		var con=3;		
		window.location="index.php?page=wholesalesarees-suit&con="+con+"&value="+val;
	}
else if(val=='')
		window.location="index.php?page=wholesalesarees-suit";
...
include('admin/connection/connection.php');
			if($_REQUEST['con']==1)
			{
					...
					$col=0;
					$price="";
					$query="SELECT * FROM tbl_suit WHERE material='$_REQUEST[value]'";
			...
			else if($_REQUEST['con']==2)
			{
					...
					$col=$_REQUEST['value'];	
					$price="";							
					 $query="SELECT * FROM tbl_suit WHERE color='$_REQUEST[value]'";
					$color_info=mysqli_fetch_array(mysqli_query($link,"SELECT * from tbl_color WHERE id='$_REQUEST[value]'"));
					$title=$color_info['color_name']." Color Salwar Suit";					 
			}
			else if($_REQUEST['con']==3)
			{
					...
					if($c[1]=="")
					{
						$query="SELECT * FROM tbl_suit WHERE price>=$c[0]";
						$title="Salwar Suit more than Rs. ".$c[0];
						$show_price="Above 3000";					 																			
					}	
					else if($c[0]==0)
					{
						$query="SELECT * FROM tbl_suit WHERE price<=$c[1]";
						$title="Salwar Suit less than Rs. ".$c[1];	
						$show_price="Less than 1000";					 																													 																			
					}					
					else
					{
						$query="SELECT * FROM tbl_suit WHERE price>=$c[0] and price<=$c[1]";
						$title="Salwar Suit in Rs. ".$c[0].' - Rs.'.$c[1];
						$show_price=$_REQUEST['value'];				 																													 																														 																									
					}	
			}
			else
			{
				$query="SELECT * FROM tbl_suit";
				$title="Salwar Suit";
			}
...
<div class="<?php echo $class; ?>">
        <div class="tour">
        <a href="index.php?page=wholesalesarees-suit-detail&product_code=<?php echo $info['product_code'];?>" title="<?php echo $info['title'];?>" >
          ...
          <div class="extra_wrapper">
            ...
            <a href="index.php?page=wholesalesarees-suit-detail&product_code=<?php echo $info['product_code'];?>" class="btn">Details</a>
          </div>
        </div>
      </div>
      <?php
        ...
        <h3>Search Suit Salwar</h3>
         <form method="post" id="form2" class="form1">
           <label >
			...
			<?php
$cat_type=mysqli_query($link,"select * from  tbl_material where product_type='2'") or die(mysql_error());
	...
	echo "<option value='$val[id]' $sel>$val[material_type]</option>";			
          ...
          <label >
            ...
            <select name="color" id="color" onchange="refreshPage(this.value,this.id)">
              <option value="" selected="selected" >All</option>
			<?php
$color=mysqli_query($link,"select * from  tbl_color where product_type='2'") or die(mysql_error());

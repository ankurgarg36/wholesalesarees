...
function refreshPage(val,id)
{
	if(val!='')
	{
		...
		var con=3;		
		window.location="index.php?page=wholesalesarees-lengha&con="+con+"&value="+val;
	}
else if(val=='')
		window.location="index.php?page=wholesalesarees-lengha";
...
<?php
			...
			else if($_REQUEST['con']==3)
			{
					...
					if($c[1]=="")
					{
						$query="SELECT * FROM tbl_lengha WHERE price>=$c[0]";
						$title="Lengha-Chunni more than Rs. ".$c[0];
						$show_price="Above 3000";					 																			
					}	
					else if($c[0]==0)
					{
						$query="SELECT * FROM tbl_lengha WHERE price<=$c[1]";
						$title="Lengha-Chunni less than Rs. ".$c[1];	
						$show_price="Less than 1000";					 																													 																			
					}					
					else
					{
						$query="SELECT * FROM tbl_lengha WHERE price>=$c[0] and price<=$c[1]";
						$title="Lengha-Chunni in Rs. ".$c[0].' - Rs.'.$c[1];
						$show_price=$_REQUEST['value'];				 																													 																														 																									
					}	
			}
			else
			{
				$query="SELECT * FROM tbl_lengha";
				$title="Lengha-Chunni";
					$page_title="WholeSaleSaree- Lengha-Chunni | online wedding Lengha-Chunni";									
			}
...
<div class="<?php echo $class; ?>">
        <div class="tour">
        <a href="index.php?page=wholesalesarees-lengha-detail&product_code=<?php echo $info['product_code'];?>" title="<?php echo $info['title'];?>" >
          ...
          <div class="extra_wrapper">
            ...
            <a href="index.php?page=wholesalesarees-lengha-detail&product_code=<?php echo $info['product_code'];?>" class="btn">Details</a>
          </div>
        </div>
      </div>
      <?php
